				...
				<form id='form-a' methode="GET">
					...
					<div class='col-md-6 col-xl-6'>
						<div class='form-group'>
							...
							<select name='produk[]' id="produk" class="form-control custom-select" multiple="multiple">
								<?php
								if ($user_categori != 8) {
								?>
								...
								if ($list_produk['num'] > 0) {
									foreach ($list_produk['results'] as $d_produk) {
										$selected = "";
										if (isset($_GET['produk_key'])) {

											if (count($_GET['produk_key']) > 1) {

												foreach ($_GET['produk_key'] as $k_produk => $v_produk) {
													if ($v_produk == $d_produk->produk_key) {
														$selected = 'selected';
													}
												}
											} else {
												$selected = ($d_produk->agentid == $_GET['produk_key'][0]) ? 'selected' : '';
											}
										}
										echo "<option value='" . $d_produk->produk_key . "' " . $selected . ">" . $d_produk->produk_key . "-" . $d_produk->produk_value . "</option>";
									}
								}
								?>
